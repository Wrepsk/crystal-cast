...
public sealed class DailymotionBrowserFrameSource : IVideoFrameSource, IMediaPlaybackTelemetrySource, IMediaPlaybackController
{
    ...
    public DailymotionBrowserFrameSource(
        string input,
        int width,
        int height,
        float captureFps,
    ...
    public string Name => activeSource?.Name ?? $"Dailymotion browser ({DescribePreference(enginePreference)})";
    ...
    {
        if (activeSource is CefDailymotionBrowserFrameSource cef)
            cef.UpdateCaptureFps(fps);
        else if (activeSource is WebView2DailymotionBrowserFrameSource webView2)
    ...
    {
        if (WebView2DailymotionBrowserFrameSource.TryGetWebView2Runtime(out var runtimeVersion, out var webView2Status))
        {
            fallbackStatus = $"using WebView2 for Dailymotion: {runtimeVersion}";
            return CreateWebView2Source();
        }

        if (CefRuntimeManager.CanInitialize(out var status))
        {
            fallbackStatus = $"WebView2 unavailable, using CEF for Dailymotion: {webView2Status}";
            return CreateCefSource();
        }

        fallbackStatus = $"browser source unavailable: WebView2 {webView2Status}; CEF {status}";
    ...
    {
        return new CefDailymotionBrowserFrameSource(input, width, height, captureFps, autoplay, currentLoop, currentPlaylistAutoplayNext, currentAudioEnabled, currentVolume, currentPlaybackRate);
    }

    private IVideoFrameSource CreateWebView2Source()
    {
        activeEngine = BrowserMediaEngine.WebView2Capture;
        return new WebView2DailymotionBrowserFrameSource(input, width, height, captureFps, autoplay, currentLoop, currentPlaylistAutoplayNext, currentAudioEnabled, currentVolume, currentPlaybackRate);
    }
    ...
    private void TryFallbackFromCefStartFailure()
    {
        ...
        if (cefStatus.Contains("invalid Dailymotion", StringComparison.OrdinalIgnoreCase))
            return;
    ...
    private void TryFallbackFromCefPlayerFailure()
    {
        if (enginePreference != BrowserMediaEngine.Auto
            || activeEngine != BrowserMediaEngine.CefOffScreen
            || activeSource is not CefDailymotionBrowserFrameSource { HasPlayerFailed: true } cefSource)
        {
            return;
        }
        ...
        activeSource.Dispose();
        activeSource = CreateWebView2Source();
        fallbackStatus = $"CEF Dailymotion playback failed, using WebView2 fallback: {cefStatus}";
    ...
    {
        public string Name => "Dailymotion browser (unavailable)";
