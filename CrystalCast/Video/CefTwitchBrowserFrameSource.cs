...
public sealed class CefTwitchBrowserFrameSource : IVideoFrameSource, IMediaPlaybackTelemetrySource, IMediaPlaybackController
{
    ...
    public CefTwitchBrowserFrameSource(
        string input,
        int width,
        int height,
        float captureFps,
    ...
    public string Name => "Twitch browser (CEF offscreen)";
    ...
    private void CreateBrowser()
    {
        ...
        catch (Exception ex)
        {
            ...
            Plugin.Log.Warning(ex, "Failed to create CEF Twitch browser.");
        }
    }
    ...
    private void LoadPlayerHtml(string reason)
    {
        ...
        var pageUrl = $"{TwitchPlayerPage.PlayerOrigin}/player.html?source={Uri.EscapeDataString(source.DisplayName)}&attempt={playerLoadAttempt}&seq={sequence}";
        var html = TwitchPlayerPage.BuildHtml(source, autoplay, audioEnabled, volume);
        ...
        browserStatus = $"CEF loading Twitch player ({reason})";
    }
    ...
    private void MaybeReloadPlayerIfNotReady()
    {
        ...
        {
            playerFailed = true;
            playerStatus = "Twitch player did not report ready";
            browserStatus = "CEF Twitch player ready timeout";
            return;
        }
        ...
        catch (Exception ex)
        {
            ...
            Plugin.Log.Debug(ex, "Failed to reload CEF Twitch player after ready timeout.");
        }
    }
    ...
    private void ExecutePlayerScript(string functionName, object? argument = null)
    {
        ...
        try
        {
            var script = argument == null
                ...
                : $"window.{functionName} && window.{functionName}({JsonSerializer.Serialize(argument, TwitchPlayerPage.JsonOptions)});";
    ...
    private void OnJavascriptMessageReceived(object? sender, object args)
    {
        try
        {
            ...
            var json = message as string ?? JsonSerializer.Serialize(message, TwitchPlayerPage.JsonOptions);
