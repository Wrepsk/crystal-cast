...
public sealed class CefDailymotionBrowserFrameSource : IVideoFrameSource, IMediaPlaybackTelemetrySource, IMediaPlaybackController
{
    ...
    private readonly DailymotionSourceReference source;
    ...
    public CefDailymotionBrowserFrameSource(
        string input,
        int width,
        int height,
        float captureFps,
    ...
    {
        this.input = input;
        isValidSource = DailymotionVideoId.TryParseSource(input, out source);
        ...
        if (!isValidSource)
            browserStatus = "invalid Dailymotion URL, video ID, or playlist";

        UpdateTelemetry(ScreenPlaybackState.Stopped, 0, 0, this.playbackRate, string.Empty);
    }

    public string Name => "Dailymotion browser (CEF offscreen)";
    ...
    public void Start()
    {
        ...
        if (!isValidSource)
        {
            browserStatus = $"invalid Dailymotion URL, video ID, or playlist: {input}";
            return;
        }
    ...
    private void CreateBrowser()
    {
        ...
        try
        {
            ...
            LoadPlayerHtml("new source");
        }
        catch (Exception ex)
        {
            browserStatus = $"CEF browser failed: {ex.Message}";
            Plugin.Log.Warning(ex, "Failed to create CEF Dailymotion browser.");
        }
    }
    ...
    private void LoadPlayerHtml(string reason)
    {
        ...
        var pageUrl = $"{DailymotionPlayerPage.PlayerOrigin}/player.html?source={Uri.EscapeDataString(source.DisplayName)}&attempt={playerLoadAttempt}&seq={sequence}";
        var html = DailymotionPlayerPage.BuildHtml(source, autoplay, loop, audioEnabled, volume);
        ...
        browserStatus = $"CEF loading Dailymotion player ({reason})";
    }
    ...
    private void MaybeReloadPlayerIfNotReady()
    {
        ...
        {
            playerFailed = true;
            playerStatus = "Dailymotion player did not report ready";
            browserStatus = "CEF Dailymotion player ready timeout";
            return;
        }
        ...
        catch (Exception ex)
        {
            ...
            Plugin.Log.Debug(ex, "Failed to reload CEF Dailymotion player after ready timeout.");
        }
    }
    ...
    private void ExecutePlayerScript(string functionName, object? argument = null)
    {
        ...
        try
        {
            var script = argument == null
                ...
                : $"window.{functionName} && window.{functionName}({JsonSerializer.Serialize(argument, DailymotionPlayerPage.JsonOptions)});";
    ...
    private void OnJavascriptMessageReceived(object? sender, object args)
    {
        try
        {
            ...
            var json = message as string ?? JsonSerializer.Serialize(message, DailymotionPlayerPage.JsonOptions);
    ...
    private void UpdateFromWebMessage(JsonElement root)
    {
        ...
        switch (type)
        {
            ...
            case "error":
                playerFailed = true;
                playerStatus = DescribeDailymotionError(root);
                break;
            case "script-error":
    ...
    private void UpdateFromStatusMessage(JsonElement root)
    {
        ...
        var audioEnabled = TryGetBool(root, "audioEnabled", this.audioEnabled);
        ...
        playerStatus = string.IsNullOrWhiteSpace(title)
            ? $"player state {stateCode}; {positionSeconds:0.0}s / {durationSeconds:0.0}s{pipDetail}{embedDetail}"
            : $"{title}; state {stateCode}; {positionSeconds:0.0}s / {durationSeconds:0.0}s{pipDetail}{embedDetail}";
    }
    ...
    {
        if (!DailymotionVideoId.IsValidVideoId(currentVideoId))
        ...
        lock (telemetryLock)
        {
            telemetry = new MediaPlaybackTelemetry
            {
                ...
                CanonicalUrl = DailymotionVideoId.BuildCanonicalSourceUrl(source, currentVideoId),
    ...
    private static string DescribeDailymotionError(JsonElement root)
    {
        if (root.TryGetProperty("message", out var messageProperty) && messageProperty.ValueKind == JsonValueKind.String)
            return messageProperty.GetString() ?? "Dailymotion player error";

        var code = TryGetInt(root, "code", 0);
        return code switch
        {
            2 => "Dailymotion player error: invalid video ID or parameter",
            5 => "Dailymotion player error: HTML5 playback failed",
            100 => "Dailymotion player error: video unavailable or private",
            101 or 150 => "Dailymotion player error: embedding is disallowed by the owner",
            153 => "Dailymotion player error: missing or blocked HTTP Referer",
            -1 => "Dailymotion player error: failed to load the player API",
            _ => $"Dailymotion player error: {code}",
        };
    }
    ...
    private static bool TryGetBool(JsonElement root, string propertyName, bool fallback)
    {
