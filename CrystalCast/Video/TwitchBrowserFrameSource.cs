...
public sealed class TwitchBrowserFrameSource : IVideoFrameSource, INativeVideoFrameSource, IMediaPlaybackTelemetrySource, IMediaPlaybackController
{
    ...
    public TwitchBrowserFrameSource(
        string input,
        int width,
        int height,
        float captureFps,
    ...
    public string Name => activeSource?.Name ?? $"Twitch browser ({DescribePreference(enginePreference)})";
    ...
    {
        if (activeSource is CefTwitchBrowserFrameSource cef)
            cef.UpdateCaptureFps(fps);
        else if (activeSource is WebView2TwitchBrowserFrameSource webView2)
    ...
    {
        if (WebView2TwitchBrowserFrameSource.TryGetWebView2Runtime(out var runtimeVersion, out var webView2Status))
        {
            fallbackStatus = $"using WebView2 for Twitch: {runtimeVersion}";
            return CreateWebView2Source();
        }

        if (CefRuntimeManager.CanInitialize(out var status))
        {
            fallbackStatus = $"WebView2 unavailable, using CEF for Twitch: {webView2Status}";
    ...
    {
        return new CefTwitchBrowserFrameSource(input, width, height, captureFps, autoplay, currentLoop, currentPlaylistAutoplayNext, currentAudioEnabled, currentVolume, currentPlaybackRate);
    }

    private IVideoFrameSource CreateWebView2Source()
    {
        activeEngine = BrowserMediaEngine.WebView2Capture;
        return new WebView2TwitchBrowserFrameSource(input, width, height, captureFps, autoplay, currentLoop, currentPlaylistAutoplayNext, currentAudioEnabled, currentVolume, currentPlaybackRate);
    }
    ...
    private IVideoFrameSource CreateWebView2WindowCaptureSource()
    {
        ...
        return new WebView2TwitchBrowserFrameSource(
            input,
            width,
            height,
            captureFps,
    ...
    private void TryFallbackFromCefStartFailure()
    {
        ...
        if (cefStatus.Contains("invalid Twitch", StringComparison.OrdinalIgnoreCase))
            return;
    ...
    private void TryFallbackFromCefPlayerFailure()
    {
        if (enginePreference != BrowserMediaEngine.Auto
            || activeEngine != BrowserMediaEngine.CefOffScreen
            || activeSource is not CefTwitchBrowserFrameSource { HasPlayerFailed: true } cefSource)
        {
            return;
        }
        ...
        activeSource.Dispose();
        activeSource = CreateWebView2Source();
        fallbackStatus = $"CEF Twitch playback failed, using WebView2 fallback: {cefStatus}";
    ...
    {
        public string Name => "Twitch browser (unavailable)";
